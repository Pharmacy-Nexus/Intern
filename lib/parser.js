...
function isContextFollowUp(text = '') {
  ...
  if (!t) return false;
  return /^(what|which|how|when|why|هل|ايه|إيه|ما|متى|ازاي|كيف|طب)\b/.test(t)
    || /\b(current|recent|latest|baseline|monitor|monitoring|symptoms|bleeding|inr|dose|labs|renal|creatinine|egfr|potassium|ضغط|الضغط|تحاليل|اعراض|أعراض|نزيف|جرعة|متابعة)\b/i.test(t);
}
